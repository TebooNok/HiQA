...
此外，表格所在的段落也应有独立的标题。
6. 转换后的每个章节中只能有一个表格，如果有多段表格，请为每一个表格另起一行，设置一个新的 # 号章节，放置这个表格，需要设置合适的章节编号和章节标题。
...
def pdf2md(kb_name, file_name):
    file_path = os.path.join(BASE_DIR, 'file_system', kb_name, 'pdfs', file_name + '.pdf')
    ...
    with open(os.path.join(BASE_DIR, 'file_system', kb_name, 'mds', file_name + '.md'), 'w', encoding='utf-8') as f:
...
#     md = pdf2md('test_kb', '模拟集成电路设计与仿真_何乐年')
#     # # save to local
